...
def stamp_duty_signature(address: str, price: float) -> str:
    normalized_address = re.sub(r"\s+", " ", address.strip().upper())
    return f"{normalized_address}|{price:.2f}" if normalized_address and price > 0 else ""


def clear_statement_price_range() -> None:
...
def sync_transient_inputs() -> None:
    st.session_state.deposit_input_mode = st.session_state.get(
        "deposit_input_mode_input",
        st.session_state.deposit_input_mode,
    )
    st.session_state.deposit_source = st.session_state.get(
        "deposit_source_input",
        st.session_state.deposit_source,
    )
    st.session_state.mortgage_1_repayment_type = st.session_state.get(
        "mortgage_1_repayment_type_input",
        st.session_state.mortgage_1_repayment_type,
    )
    st.session_state.mortgage_2_repayment_type = st.session_state.get(
        "mortgage_2_repayment_type_input",
        st.session_state.mortgage_2_repayment_type,
    )
    st.session_state.mortgage_3_repayment_type = st.session_state.get(
        "mortgage_3_repayment_type_input",
        st.session_state.mortgage_3_repayment_type,
    )
    st.session_state.listing_url = st.session_state.get(
        "listing_url_input",
        st.session_state.listing_url,
    )
    st.session_state.listing_text = st.session_state.get(
        "listing_text_input",
...
def fetch_statement_price_range(force=False):
    ...
    previous_lookup_address = st.session_state.get("last_soi_lookup_address", "").strip()
    address_changed = addr != previous_lookup_address
    if state != "VIC":
        if address_changed:
            clear_statement_price_range()
        st.session_state.statement_lookup_message = "SOI only applies to VIC."
        st.session_state.last_soi_lookup_address = addr
        return
...
def refresh_stamp_duty():
    ...
    addr = st.session_state.property_address.strip()
    state = parse_state_from_address(addr)
    auto_signature = st.session_state.get("stamp_duty_auto_signature", "")
    current_signature = stamp_duty_signature(addr, price)
    ...
    st.session_state.stamp_duty_auto_signature = current_signature if result["supported"] else ""
...
def render_tabs(payload: dict):
    ...
    with tab_details:
        ...
        cC, cD = st.columns(2)
        cC.button("Search verified REA SOI", on_click=fetch_statement_price_range, use_container_width=True)
        cD.button("Refresh stamp duty", on_click=refresh_stamp_duty, use_container_width=True)

        if st.session_state.listing_import_message:
            st.info(st.session_state.listing_import_message)
        if st.session_state.statement_lookup_message:
            st.info(st.session_state.statement_lookup_message)
        if st.session_state.statement_source_url:
            st.markdown(f"[Open SOI source]({st.session_state.statement_source_url})")
    ...
    with tab_export:
        ...
        if st.button("Save property", use_container_width=True):
            payload = current_payload()
            name = st.session_state.save_name_input_export or payload.get("property_address") or "Property"
            save_property(
                name=name,
...
def main():
    ...
    # Main Title
    st.title("🏠 Property Check – Premium Investment Dashboard")
